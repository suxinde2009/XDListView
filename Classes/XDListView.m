 ...
 */
- (void)setNeedsReload {
    NSNotification *notification = [NSNotification notificationWithName:XDListViewSetNeedsReloadNotification
                                                 object:self];
...
- (NSCache*)cellCache {
    if( !mCellsCache ) {
        ...
        [mCellsCache setName:@"com.XDListView.cellCache"];
    }
    return mCellsCache;
}
...
#pragma mark - XDListViewDelegate wrapper methods
/**
 *
 */
- (void)willDisplayCell:(id)aCell
          forRowAtIndex:(NSUInteger) aRow {
...
#pragma mark - XDListViewDataSource wrapper methods
/**
...
- (id)cellForRowAtIndex:(NSUInteger)aRow {
    ...
    if( [[self dataSource] respondsToSelector:@selector(listView:cellForRowAtIndex:)] ) {
        ...
        if( (cellIdentifier = [cell reuseIdentifier]) ) {
            [[self cellCacheForIdentifier:cellIdentifier] __listEnqueue:cell];
        }
